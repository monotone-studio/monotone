...
Cmd*
parse_partition_sync(Lex* self)
{
	// SYNC PARTITION [IF EXISTS] <id>
	...
	Token id;
	if (! lex_if(self, KINT, &id))
		error("SYNC PARTITION <id>");

	auto cmd = cmd_partition_allocate(CMD_PARTITION_SYNC);
	cmd->min       = id.integer;
	cmd->if_exists = if_exists;
	cmd->if_cloud  = true;
	return &cmd->cmd;
...
Cmd*
parse_partition_sync_range(Lex* self)
{
	// SYNC PARTITIONS FROM <min> TO <max>

	// from
	if (! lex_if(self, KFROM, NULL))
		error("SYNC PARTITIONS <FROM> min TO max");
	...
	auto cmd = cmd_partition_allocate(CMD_PARTITION_SYNC_RANGE);
	cmd->min      = min.integer;
	cmd->max      = max.integer;
	cmd->if_cloud = true;
	return &cmd->cmd;
